# Week 7 - CSS Selectors

## Treehouse courses due this week
- [CSS Selectors](https://teamtreehouse.com/library/css-selectors)

## Treehouse courses due by next class
- [Web Accessibility Compliance](https://teamtreehouse.com/library/web-accessibility-compliance)
- [Website Optimization](https://teamtreehouse.com/library/website-optimization)

## CSS Selector Resources
- [CSS Diner](https://flukeout.github.io/)
- [W3Schools CSS Selectors](https://www.w3schools.com/css/css_selectors.asp)
- [W3Schools CSS Selector Reference](https://www.w3schools.com/cssref/css_selectors.asp)
- [MDN CSS Selectors](https://developer.mozilla.org/en-US/docs/Web/CSS/CSS_Selectors)
- [MDN CSS Building Blocks: CSS Selectors](https://developer.mozilla.org/en-US/docs/Learn/CSS/Building_blocks/Selectors)
- [CSS Tricks - Beginner Concepts: How CSS Selectors Work](https://css-tricks.com/how-css-selectors-work/)
- [HTML Dog: CSS Selectors](https://htmldog.com/references/css/selectors/)
- [W3 CSS Selector Standards](https://www.w3.org/TR/CSS21/selector.html%23id-selectors)
- [The 30 CSS Selectors You Must Memorize](https://code.tutsplus.com/tutorials/the-30-css-selectors-you-must-memorize--net-16048)
- [Understand ‘+’, ‘>’ and ‘~’ symbols in CSS Selector](https://techbrij.com/css-selector-adjacent-child-sibling)