## Table of Contents
- [Non-Treehouse Resources](#Non-Treehouse-Resources)
   - [Free Images and Icons](#Free-Images-and-Icons)
   - [Documentation](#documentation)
   - [Tutorials and Courses](#tutorials-and-courses)
- [Software](#software)
   - [IDEs and Text Editors](#IDEs-and-Text-Editors)
   - [Terminals](#Terminals)
   - [Source Control GUI](#Source-Control-GUI)
   - [Additional Software](#Additional-Software)

## Treehouse Resources
### Treehouse Courses
- [Introducing the VS Code Text Editors](https://teamtreehouse.com/library/introducing-the-vs-code-text-editor)
- [CSS Layout Basics](https://teamtreehouse.com/library/css-layout-basics)
- [Introduction to the Terminal](https://teamtreehouse.com/library/introduction-to-the-terminal)
- [Introduction to Git](https://teamtreehouse.com/library/introduction-to-git)
- [GitHub Basics](https://teamtreehouse.com/library/github-basics)
- [Debugging CSS with Chrome DevTools](https://teamtreehouse.com/library/debugging-css-with-chrome-devtools)
- [Responsive Layouts](https://teamtreehouse.com/library/responsive-layouts)
- [Practice Media Queries](https://teamtreehouse.com/library/practice-media-queries)
- [CSS Flexbox Layout](https://teamtreehouse.com/library/css-flexbox-layout)
- [HTML Forms](https://teamtreehouse.com/library/html-forms)
- [Practice Basic Variables, Input, and Output in JavaScript](https://teamtreehouse.com/library/practice-basic-variables-input-output-in-javascript)
- [JavaScript Numbers](https://teamtreehouse.com/library/javascript-numbers)
- [JavaScript Functions](https://teamtreehouse.com/library/javascript-functions)
- [JavaScript Loops, Arrays, and Objects](https://teamtreehouse.com/library/javascript-loops-arrays-and-objects)
- [CSS Selectors](https://teamtreehouse.com/library/css-selectors)

## Non-Treehouse Resources
### Free Images and Icons
- [Foundation Icon Fonts](https://zurb.com/playground/foundation-icon-fonts-3)
- [Font Awesome](https://fontawesome.com/)

### Documentation
- [Atlassian Git Command Cheatsheet](https://confluence.atlassian.com/bitbucketserver/basic-git-commands-776639767.html)
- [Bootstrap](https://getbootstrap.com/)
- [Codeburst JavaScript Functions](https://codeburst.io/javascript-functions-understanding-the-basics-207dbf42ed99)
- [DevDojo](https://devdojo.com)
   - [DevDojo HTML Forms](https://devdojo.com/html/form-elements)
   - [DevDojo HTML Form Elements](https://devdojo.com/html/form-elements)
- [Digital Ocean - How To Do Math in JavaScript with Operators](https://www.digitalocean.com/community/tutorials/how-to-do-math-in-javascript-with-operators)
- [Eloquent JavaScript - 03: Functions](https://eloquentjavascript.net/03_functions.html)
- [GitHub Markdown Cheatsheet PDF](https://guides.github.com/pdfs/markdown-cheatsheet-online.pdf)
- [HTMLReference.io Forms in HTML5](https://htmlreference.io/forms/)
- [JavaScript.info](https://javascript.info/)
   - [JavaScript.Info Form Properties and Methods](https://javascript.info/form-elements)
   - [JavaScript.info Variables](https://javascript.info/variables)
   - [JavaScript.info Functions](https://javascript.info/function-basics)
   - [JavaScript.Info Arrow functions, the basics](https://javascript.info/arrow-functions-basics)
- [JavaScriptTutorial.net](https://www.javascripttutorial.net/)
   - [JavaScript Tutorial: JavaScript Variables](https://www.javascripttutorial.net/javascript-variables/)
   - [JavaScript Tutorial: JavaScript Function](https://www.javascripttutorial.net/javascript-function/)
- [Markdown Guide Cheatsheet](https://www.markdownguide.org/cheat-sheet/)
- [Mozilla Developer Network aka MDN](https://developer.mozilla.org/en-US/)
   - [Mozilla Developer Network <Form> Section](https://developer.mozilla.org/en-US/docs/Web/HTML/Element/form)
   - [Mozilla Developer Network var](https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Statements/var)
   - [Mozilla Developer Network Basic Math in JavaScript](https://developer.mozilla.org/en-US/docs/Learn/JavaScript/First_steps/Math)
   - [MDN JavaScript Guide: Functions](https://developer.mozilla.org/en-US/docs/Web/JavaScript/Guide/Functions)
   - [MDN JavaScript Reference: Functions](https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Functions)
- [TutorialsPoint JavaScript Functions](https://www.tutorialspoint.com/javascript/javascript_functions.htm)
- [TutorialsTeacher ](https://www.tutorialsteacher.com/)
   - [TutorialsTeacher JavaScript Variable](https://www.tutorialsteacher.com/javascript/javascript-variable)
   - [TutorialsTeacher JavaScript Functions](https://www.tutorialsteacher.com/javascript/javascript-function)
- [W3Schools](https://www.w3schools.com/)
   - [W3CSchools CSS Flexbox page](https://www.w3schools.com/css/css3_flexbox.asp)
   - [W3CSchools RWD Intro](https://www.w3schools.com/css/css_rwd_intro.asp)
   - [W3CSchools CSS Media Queries Page](https://www.w3schools.com/css/css3_mediaqueries.asp)
   - [W3Schools HTML Forms Section](https://www.w3schools.com/html/html_forms.asp)
   - [W3Schools JavaScript Variables](https://www.w3schools.com/js/js_variables.asp)
   - [W3Schools JavaScript Operators](https://www.w3schools.com/js/js_operators.asp)
   - [W3Schools JavaScript Arithmetic](https://www.w3schools.com/js/js_arithmetic.asp)
   - [W3Schools JavaScript Math Object](https://www.w3schools.com/js/js_math.asp)
   - [W3Schools JavaScript Functions](https://www.w3schools.com/js/js_functions.asp)
   - [W3Schools JavaScript Function Definition](https://www.w3schools.com/js/js_function_definition.asp)
- [World Wide Web Consortium (W3C)](https://www.w3.org/)
   - [The W3 Standards for Forms](https://www.w3.org/TR/html4/interact/forms.html)

### Tutorials and Courses
- [Freshman - How to build a Calculator App with JavaScript](https://freshman.tech/calculator/)
- [FreeCodeCamp](https://www.freecodecamp.org/)
   - [FreeCodeCamp - How to build an HTML calculator app from scratch using JavaScript](https://www.freecodecamp.org/news/how-to-build-an-html-calculator-app-from-scratch-using-javascript-4454b8714b98/)
   - [FreeCodeCamp - How to Discard Unstaged Changes in Git](https://www.freecodecamp.org/forum/t/how-to-discard-unstaged-changes-in-git/13214)
- [W3C - Declaring language in HTML](https://www.w3.org/International/questions/qa-html-language-declarations)
- [Flexbox Froggy](https://flexboxfroggy.com/)

### Software
#### IDEs and Text Editors
- [Visual Studio Code aka VS Code](https://code.visualstudio.com/)
- [Visual Studio Community](https://visualstudio.microsoft.com/vs/community/)
- [NotePad++](https://notepad-plus-plus.org/downloads/)
- [ATOM IDE](https://atom.io/)
#### Terminals
- [Git for Windows](https://gitforwindows.org/)
#### Source Control GUI
- [GitHub for Desktop](https://desktop.github.com/)
- [SourceTree](https://www.sourcetreeapp.com/)
#### Additional Software
- [Postman](https://www.postman.com/) - Used for testing API calls. More for when you get into advanced courses and start including an API.
- [Azure Data Studio](https://docs.microsoft.com/en-us/sql/azure-data-studio/what-is?view=sql-server-ver15) - Serves multiple purposes. Allow you to connect to SQL databases and create notebooks where you can store scripts just for notes or to actually run within the program.
- [LINQPad](https://www.linqpad.net/) - Similar to Azure Data Studio, but with a focus on LINQ and SQL. You can store scripts in it and run them against various databases.
